...
type Service struct {
	...
	subStageSVC adminlib.SupplierStageService
}
...
func main() {
	...
	subStageSVC := adminlib.CreateSupplierStageService(ctx, ddbclient, logger)
	subStageSVC.SupplierStagesTable = os.Getenv("SUPPLIER_STAGES_TABLE")
	subStageSVC.SupplierStages_SupplierIdIndex = os.Getenv("SUPPLIER_STAGES_INDEX_SUPPLIERID")
...
type GetSupplierStagesResponse struct {
...
/*
SupplierId is passed in the request Headers from the frontEnd.
*/
func (svc *Service) GETRequestHandler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {

	// 1. Get SupplierId from the Header
	SupplierId := request.Headers["supplierid"]

	// 2. Get all Supplier related Stages from the supplier stage table

	stageData, err := svc.subStageSVC.GetAllSupplierStages(SupplierId)
	if err != nil {
		return events.APIGatewayProxyResponse{StatusCode: 500}, err
	}
...
func MapStageDataToResponse(allStageData []adminlib.SupplierStages) GetSupplierStagesResponse {

	var apiResp GetSupplierStagesResponse
...
func SetOverallStatus(response *GetSupplierStagesResponse) {
